 ...
 * for inclusion in an IntervalMap
 *
 * The class you use as node within the ItervalMap *must* derive from this class
 * template. I supplies your node class with the necessary members to contain
 * the metainformation for managing the underlying RBTree.
 *
 * See the IntervalMap for more documentation on keys, values, tags and how the
 * IntervalMap behaves.
 *
 * @tparam KeyT			The type of the keys in the IntervalMap.
 * @tparam ValueT		The type of the values in the IntervalMap. Must be
 * default-constructible, addable and subtractable.
 * @tparam Tag			The tag used to identify the underlying RBTree. If
 * you want your nodes to be part of multiple IntervalMaps, RBTrees or
 * IntervalTrees, each must have its own unique tag. Can be any class, the class
 * can be empty.
 */
...
public:
  /**
   * Inserting nodes into an IntervalMap results in the key space to be divided
   * into Segments. See DOCTODO for details and examples. This is the type that
   * these segments will have.
   */
 ...
 * telling the IntervalMap how to interact with your node class.
 *
 * You must derive from this class template and supply the IntervalMap with your
 * own derived class. At the least, you have to implement the methods get_lower,
 * get_upper and get_value for the IntervalMap to work. See the respective
...
public:
  ...
  /**
   * Callback that is called when the aggregate value for a segment changes. See
   * DOCTODO for information on segments.
   *
  ...
  /**
   * Callback that is called when the length of a segment changes. See TODO for
   * how to retrieve the length of a segment; especially note that there are
   * zero-length segments. See DOCTODO for information on segments.
   *
   * Note that this will also be called on newly inserted segments; the
   * aggregate value for those segments will then not be updated yet.
   *
  ...
  /**
   * Callback that is called when a new segment was created. Note that at the
   * point this callback is called, the aggregate value associated with the
   * segment is not yet determined. See DOCTODO for information on segments.
   *
  ...
  /**
   * Callback that is called when a segment is destroyed. See DOCTODO for
...
/**
 * @brief An IntervalMap stores a collection of intervals that are associated
 * with a value and allows for efficient access to aggregate values
 *
 * An IntervalMap stores a collection of intervals that are associated with a
 * value. Where multiple intervals in the interval map overlap, values are
 * aggregated (e.g., by adding them up). The interval map then allows to
 * efficiently query for the aggregated value at a certain point, to iterate the
 * whole "horizon" of intervals and their respective values, and so on.
 *
 * To this end, the whole "horizon", that is, the space between the smallest
 * lower interval border and the largest upper interval border in the map, is
 * divided into segments. A segment represents a contiuous interval (note: *not*
 * necessarily one of the intervals inserted into the map!) during which the
 * aggregate value does not change. Note that with n intervals inserted into the
 * map, there are at most 2n - 1 such segments. In fact, for implementation
 * reasons, there will always be exactly 2n - 1 segments in an IntervalMap.
 * However, where multiple intervals start or end at the same point, segments of
 * length 0 occurr.
 *
 * @tparam Node					The node class for the interval map.
 * Must be derived from IMapNodeBase.
 * @tparam NodeTraits		The node traits, mainly defining how the
 * IntervalMap retrieves data from your nodes. Must be derived from
 * IMapNodeTraits.
 * @tparam Options			The TreeOptions class specifying the parameters
 * of this IntervalMap. See the TreeOptions and TreeFlags classes for details.
 * @tparam Tag					The tag used to identify the underlying
 * RBTree. If you want your nodes to be part of multiple IntervalMaps, RBTrees
 * or IntervalTrees, each must have its own unique tag. Can be any class, the
 * class can be empty.
 */
...
public:
  /**
   * @brief A segment is a continuous range during which the aggregated value
   * does not change.
   *
   * Not that at the start and end of every segment, at least one interval
   * starts or ends. However, not all interval starts and ends create a segment
   * border, because two consecutive intervals might have the same value, thus
   * resulting in a single segment.
   *
   * @note Internally, the IntervalMap contains more segments than described
   * above. However, the IntervalMap API presents the segments as described
   * above.
   */
  ...
  using ITree =
      RBTree<Segment, RBDefaultNodeTraits<Segment>,
