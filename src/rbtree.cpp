...
template<class Node>
void
ColorParentStorage<Node, false>::swap_color_with(ColorParentStorage<Node, false> & other)
{
...
template<class Node>
void
ColorParentStorage<Node, false>::swap_parent_with(ColorParentStorage<Node, false> & other)
{
...
RBTree<Node, NodeTraits, Options, Tag, Compare>::fixup_after_insert(Node *node)
{
  ...
  */
  ...
  if (node->NB::get_parent()->NB::get_color() == rbtree_internal::Color::BLACK) {
    return;
  }
...
RBTree<Node, NodeTraits, Options, Tag, Compare>::insert(Node &node, Node &hint)
{
   ...
   */
  while ((parent->NB::get_parent() != nullptr) &&
         (((parent->NB::get_parent()->NB::_rbt_left == parent) &&
           (this->cmp(*parent->NB::get_parent(),
                      node))) || // left subtree, parent should go before node
          ((parent->NB::get_parent()->NB::_rbt_right == parent) && (this->cmp(node,
                                                                             *parent->NB::get_parent()))))) { // right subtree, node should go before parent
...
{
  return ((this->root == nullptr) || (this->root->NB::get_color() == rbtree_internal::Color::BLACK));
}
...
RBTree<Node, NodeTraits, Options, Tag, Compare>::verify_red_black(const Node *node) const
{
  ...
  if (node->NB::get_color() == rbtree_internal::Color::RED) {
    if ((node->NB::_rbt_right != nullptr) &&
        (node->NB::_rbt_right->NB::get_color() == rbtree_internal::Color::RED)) {
      return false;
    }
                                                              ...
                                                              reverse>::step_forward()
{
  ...
  if (this->n->NB::_rbt_right != nullptr) {
    ...
    // skip over the nodes already visited
    while ((this->n->NB::get_parent() != nullptr) && (this->n->NB::get_parent()->NB::_rbt_right ==
                                                     this->n)) { // these are the nodes which are smaller and were already visited
      this->n = this->n->NB::get_parent();
    }

    // go one further up
    if (this->n->NB::get_parent() == nullptr) {
      // done
      this->n = nullptr;
    } else {
      // go up
                                                              ...
                                                              reverse>::step_back()
{
  if (this->n->NB::_rbt_left != nullptr) {
    ...
    // skip over the nodes already visited
    while ((this->n->NB::get_parent() != nullptr) && (this->n->NB::get_parent()->NB::_rbt_left ==
...
template <class Comparable>
typename RBTree<Node, NodeTraits, Options, Tag, Compare>::template iterator<false>
RBTree<Node, NodeTraits, Options, Tag, Compare>::lower_bound(const Comparable &query)
{
  Node *cur = this->root;
  Node *last_left = nullptr;
