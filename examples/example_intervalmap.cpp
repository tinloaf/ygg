...
/* The node class. It provides a simple interval -> value mapping, where
 * interval borders as well as the value associated with an interval are
 * integers.
 */
...
/* We need to specify our own NodeTraits. This class is used by the IntervalMap
 * to retrieve the interval borders as well as the value from the nodes.
 */
...
main(int argc, char ** argv)
{
  ...
  for (auto it = m.begin(); it != m.end(); ++it) {
    std::cout << "Segment from " << it.get_lower() << " to " << it.get_upper()
              << ": aggregate "
                 "value "
              << it.get_value() << "\n";
  }
