...
public:
	...
	double get_average_depth() const noexcept {
		return static_cast<double>(this->depth_sum) / static_cast<double>(this->nodes.size());
	}

	size_t get_max_depth() const noexcept {
...
private:
	...
	void build() {
		for (auto & node : nodes) {
			std::uniform_int_distribution bound_distr(std::numeric_limits<int>::min(),
